<form action="{{ route('task.update', $main_task->id) }}" method="post">
    @method('put')
    @csrf
    <div class="mb-3">
        <label for="role">Client</label>
        <select name="client_id" id="client_id" class="form-control" required>
            <option  style ="display:none" value="">Selected Client</option>
            @foreach ($clients as $client)
                <option {{ $client->id == $main_task->client_id ? 'selected' : '' }} value="{{ $client->id }}">
                    {{ $client->name }}</option>
            @endforeach
        </select>
    </div>
    <div class="mb-3">
        ...
        <select name="assigned_to" id="assigned_to" class="form-control" required>
            ...
            @foreach ($users as $user)
                <option {{ $user->id == $main_task->assigned_to ? 'selected' : '' }} value="{{ $user->id }}">
                    {{ $user->name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" value="{{ $main_task->title }}" name="title" id="name"
    ...
    <div class="mb-3">
        ...
        required>{{ $main_task->description }}</textarea>
    </div>
    ...
    <div class="text-center">
        <button class="btn btn-info" type="submit"> Update Main Task </button>
    </div>
</form>
